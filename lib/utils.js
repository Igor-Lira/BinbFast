 ...
 */
 ...
 exports.buildLeaderboardsTracksForUsers = function(tracks) {
  ...
  for (let i = 0; i < tracks.length; i+=1){
    tracksToRender[i] = ['song name', 30];
    if (tracks[i].indexOf('(') > 0){
      tracksToRender[i][0] = tracks[i][0].substr(0, tracks[i].indexOf('('));
    } else{
      tracksToRender[i][0] = tracks[i][0];
    }
    tracksToRender[i][1] = (tracks[i][1]/1000).toFixed(3);
  }
  return tracksToRender;
};
