...
/**
 * Helper function used to build leaderboards for tracks.
 * Rearrange database results in an object.
 */

 exports.buildLeaderboardsTracks = function(tracks) {
  let tracksToRender = tracks;
  for (let i = 0; i < tracksToRender.length; i++){
    tracksToRender[i][1] = (tracksToRender[i][1]/1000).toFixed(3);
    if (tracksToRender[i][0].indexOf('(') > 0){
      tracksToRender[i][0] = tracksToRender[i][0].substr(0, tracksToRender[i][0].indexOf('('));
    }
  }
...
/**
 * Helper function used to build score of tracks by user.
 * Rearrange database results in an object.
 */

 exports.buildLeaderboardsTracksForUsers = function(tracks) {
  let tracksToRender = [[]];
  for (let i = 0; i < tracks.length/2; i=i+2){
    let j = i/2;
    tracksToRender[j] = ['song name', 30];
    if (tracks[i].indexOf('(') > 0){
      tracksToRender[j][0] = tracks[i].substr(0, tracks[i].indexOf('('));
    } else{
      tracksToRender[j][0] = tracks[i];
    }
    tracksToRender[j][1] = (tracks[i+1]/1000).toFixed(3);
  }
  return tracksToRender;
};
