...
const updateSongStats = require('./tracks').trackStats;
...
function Room(roomname) {
  ...
  if (["hits", "rap", "rock", "oldies"].includes(this.roomname)){
    this.totalPlayingTime = 1000*config.songduration;
  } else if (this.roomname === "covers"){
    this.totalPlayingTime = 30000;
  } else {
    this.totalPlayingTime = 10000;
  }
  this.userGuesses = [];
  this.initialize();
}
...
Room.prototype.addPointsAndStats = function(nickname, allinone, guess) {
  const stats = {};
  ...
  userData.guessed++;
  userData.guesstime = this.totalPlayingTime - this.songtimeleft;
  ...
  if (userData.registered) {
    ...
    updateSongStats(userStats, this.trackId, guess);
  }
};
...
Room.prototype.gameOver = function() {
  ...
  setTimeout(this.sendLoadTrack.bind(this), 1000);
};
...
Room.prototype.onGuess = function(spark, guess) {
  ...
  if (this.userGuesses[spark.nickname] === undefined){
    this.userGuesses[spark.nickname] = { artist: '', title: ''}
  }
  ...
  if (!userData.matched) {
    ...
    if (amatch(artist, guess, true) || (feat && amatch(feat, guess, true))) {
      this.userGuesses[spark.nickname].artist = guess;
      return this.onMatch(spark, 'artist', guess);
    }
    if (amatch(title, guess)) {
      this.userGuesses[spark.nickname].title = guess;
      return this.onMatch(spark, 'title', guess);
    }
...
Room.prototype.onMatch = function(spark, what, guess) {
  const nickname = spark.nickname;
  const usersData = this.usersData;
...
Room.prototype.onPair = function(spark, allinone) {
  this.addPointsAndStats(spark.nickname, allinone, this.userGuesses[spark.nickname]);
  ...
  songsdb.zrange([this.roomname, index, index], function(err, res) {
    ...
    songsdb.hmget(
      ...
      function(err, replies) {
        ...
        room.bestScore = replies[5];
        room.userBestScore = replies[6];
        room.trackId = id; 
        primus.room(room.roomname).send('loadtrack', room.previewUrl);

        setTimeout(room.sendPlayTrack.bind(room), 1000);
      }
    );
  });
};
...
Room.prototype.sendPlayTrack = function() {
  ...
  this.startTimer(Date.now() + this.totalPlayingTime, 50);
  setTimeout(this.sendTrackInfo.bind(this), this.totalPlayingTime);
};
...
Room.prototype.sendTrackInfo = function() {
  primus.room(this.roomname).send('trackinfo', {
    ...
    userBestScore: this.userBestScore
  });
