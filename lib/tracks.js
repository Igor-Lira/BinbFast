    ...
    songsdb.hmget(
        [
            ...
            if (userStats.guesstime && userStats.guesstime < bestScore) {
                ...
                songsdb.zadd ('rap4', userStats.guesstime,  trackId);
            }
        })
        /*
       ...
       // console.log ("|============RANK===========|")
        for (let i = 0; i < tracks.length; i++){
            songsdb.hmget ([tracks[i], 'trackName',  'bestScore', 'userBestScore'], (err, replies)=>{
                 const inSec = replies[1]/1000;
                 //console.log (`${i}: ${replies[0]} - ${inSec}`);
                 rankByTrack.push(`${i}: ${replies[0]} - ${inSec}`);
                 callback(rankByTrack);
             })
        }
    });
    multi.exec(()=>{
        console.log (rankByTrack);
        callback(rankByTrack);
    })
    */
    
     songsdb.zrange(['rap4', 0, 30],  (err, tracks) => {
